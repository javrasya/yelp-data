  ...
  def doSampling(sparkSession: SparkSession, fullDataPath: String): Unit = {
    ...
    val reviewDF = sparkSession.sql(
      s"""
        ...
        """.stripMargin).filter($"rank" < 10).drop("rank")
    ...
    s"cat ${new File(resourceDataPath, "yelp_academic_dataset_business//*.json").getPath} >> ${new File(resourceDataPath, "yelp_academic_dataset_business.json").getPath}" !

    s"cat ${new File(resourceDataPath, "yelp_academic_dataset_review//*.json").getPath} >> ${new File(resourceDataPath, "yelp_academic_dataset_review.json").getPath}" !

    s"cat ${new File(resourceDataPath, "yelp_academic_dataset_user//*.json").getPath} >> ${new File(resourceDataPath, "yelp_academic_dataset_user.json").getPath}" !

    s"cat ${new File(resourceDataPath, "yelp_academic_dataset_tip//*.json").getPath} >> ${new File(resourceDataPath, "yelp_academic_dataset_tip.json").getPath}" !

    s"cat ${new File(resourceDataPath, "yelp_academic_dataset_checkin//*.json").getPath} >> ${new File(resourceDataPath, "yelp_academic_dataset_checkin.json").getPath}" !
